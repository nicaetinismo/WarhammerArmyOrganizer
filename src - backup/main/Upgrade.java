...
public class Upgrade {
	...
	public void makeEntry(JPanel p, int x, int y, boolean edit){
		...
		if(!(maxInSquad == -1) && !(maxInSquad == 1)){
			...
			c.weightx = 0.2;
			if(p == null) file.upgradePanel.add(spinner, c);
			else p.add(spinner, c);
			c.gridx = x+1;
			c.weightx = 0.8;
			if(p == null) file.upgradePanel.add(label, c);
			else p.add(label, c);
		}
		else {
			c.weightx = 1.0;
			...
			c.weightx = 0.1;
			if(p == null) file.upgradePanel.add(checkBox, c);
			...
			c.gridx = x+1;
			c.weightx = 0.8;
			if(p == null) file.upgradePanel.add(temp, c);
			else p.add(temp, c);
		}
	}
	...
	public void updateCostBasedOnSize(){
		if(maxInSquad == -1){
			numberInSquad = file.currentSize;
		}
		else if(linkedUpgrades.isEmpty() && file.currentSize < numberInSquad){
			if(spinner != null){
				((SpinnerNumberModel) spinner.getModel()).setValue(new Integer(file.currentSize));
			}
		}
	}
	...
	protected void setActive(boolean b){
		isActive = b;
		if(b) file.addUpgrade(this);
		else file.removeUpgrade(this);
		file.update();
	}
	...
	public int getMax(){
		int temp = 0;
		if(maxInSquad == -1) return file.currentSize;
		else if(linkedUpgrades.isEmpty()){
			temp = Integer.valueOf(maxInSquad);
			if(temp <= file.currentSize){
				if(requiredNumber > 1){
					if(file.currentSize/requiredNumber < temp) return (int) (file.currentSize/requiredNumber);
					else return temp;
				}
				else return temp;
			}
			else return (int) ( file.currentSize/requiredNumber);
		}
		else {
			if(Integer.valueOf(maxInSquad) <= file.currentSize) temp += Integer.valueOf(maxInSquad);
			else temp += file.currentSize;
		}
		for(String s: linkedUpgrades){
			for(Upgrade u: file.currentUpgrades){
